...
javascript: (function() {
  const popupCss = `
  .popup { background-color: white; border: 2px solid #444; border-radius: 10px; width: 700px; height: 600px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.5); position: relative; }
  ...
  #section1, #section2 { text-align: center; height: 220px; }
  #section1 input, #section2 input { margin-bottom: 10px; width: 230px;}
  #section1 #userList, #section2 #userList2 { margin-bottom: 15px; max-height: 130px; overflow-y: scroll; scrollbar-width: none; -ms-overflow-style: none; }
  #section1 #userList::-webkit-scrollbar, #section2 #userList2::-webkit-scrollbar { display: none; }
  #sectionsRow { white-space: nowrap; }
  #businessUnitList li, #teamsList li, #section3 ul li, #section4 ul li { margin-left: 20px; }
  #businessUnitList, #businessUnitList2 { margin-bottom: 15px; }
`;
...
function createAndAppendPopup() {
  ...
  function renderUserList(users, selectUserCallback, userListDivId) {
    const userListDiv = document.getElementById(userListDivId);
    users.forEach(user => {
